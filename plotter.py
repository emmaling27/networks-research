...
class Plotter():
    ...
    def __init__(self, count_path, plots_path, start, bucket_size=1):
        """
        count_path : str
            path for the directory with pickled count dicts
        plots_path : str
            path where plots generated by this class should be stored
        start : time period to start plots at [start, ...]
        bucket_size : size of time period interval to bucket by for marginal counts
        """
    ...
    def get_marginal_counts(self, arr):
        """Return an array of marginal counts from an array of cumulative counts"""
        marginal_counts = []
    ...
    def plot_nodes_over_time(self, counts, name):
        """Plot nodes in the graph over time"""
        plt.plot(
    ...
    def plot_edges_over_time(self, counts, name):
        """Plot edges in the graph over time"""
        plt.plot(
    ...
    def plot_bichromatic_fraction_diff_over_time(self, counts, name):
        """Plot the difference between cumulative bichromatic fractions by type and the overall bichromatic fraction"""
        plt.plot(
            range(self.start, len(counts['f(w)'])), 
    ...
    def plot_marginal_w_b_over_time(self, counts, name):
        """Plot w_b/w and b_b/b over time"""
    ...
    def plot_marginal_bichromatic_fraction_diff_over_time(self, counts, name):
        """Plot w_b/w - f(e) and b_b/b - f(e) over time"""
        plt.plot(
    ...
    def plot_all(self):
        """Plot all the above for all counts in the count directory"""
